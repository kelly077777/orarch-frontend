        ...
        {open && (
          <div style={{
            ...
            {[{ key: 'org', label: 'BY ORGANISATION' }, { key: 'list', label: 'LIST' }].map(opt => (
              <button key={opt.key} onClick={() => { setView(opt.key); setOpen(false); }}
                style={{
                  width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  padding: '10px 14px', background: 'none', border: 'none',
                  fontSize: '12px', fontWeight: 700, color: '#475569', cursor: 'pointer', letterSpacing: '0.04em',
                }}
  ...
  return (
    <div onClick={onClick}
      ...
      style={{
        ...
        boxShadow: hovered ? '0 4px 20px rgba(0,0,0,0.08)' : 'none', transition: 'box-shadow 0.2s',
      }}>
      <div style={{ height: '4px', background: color }} />
      <div style={{ padding: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
          <div style={{ fontSize: '14px', fontWeight: 700, color: '#1E293B' }}>{project.name}</div>
          <span style={{ background: statusBg[project.status]||'#F1F5F9', color: statusColor[project.status]||'#64748B', fontSize: '10px', fontWeight: 600, padding: '2px 8px', borderRadius: '20px' }}>
            {project.status}
          </span>
        </div>
...
function OrgView({ projects: list, onSelect }) {
  const groups = {};
  list.forEach(p => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', fontFamily: 'Arial,sans-serif' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, background: '#F8FAFC', overflowY: 'auto' }}>
          ...
          {/* Top bar */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 28px', borderBottom: '1px solid #E2E8F0', background: '#fff' }}>
            <div style={{ position: 'relative', width: '280px' }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Filter projects"
                style={{ width: '100%', padding: '7px 12px 7px 30px', border: '1px solid #E2E8F0', borderRadius: '6px', fontSize: '13px', outline: 'none', background: '#F8FAFC', boxSizing: 'border-box', color: '#1E293B' }} />
            </div>
          ...
          <div style={{ padding: '24px 28px' }}>
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '60px', color: '#94A3B8' }}>
                ...
                <div style={{ fontSize: '13px' }}>Ask your admin to create a project for you</div>
              </div>
            ) : view === 'org' ? (
              <OrgView projects={filtered} onSelect={handleSelect} />
            ) : view === 'list' ? (
              <div style={{ border: '1px solid #E2E8F0', borderRadius: '10px', overflow: 'hidden', background: '#fff' }}>
                {filtered.map((p, i) => (
                  <div key={p.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                    <ProjectRow project={p} onClick={() => handleSelect(p)} />
                  </div>
                ))}
              </div>
            ) : (
